...
| LAB-07 | O motor do laboratório de parcelamento na esteira: ida e volta pelo contrato de motor v1, julgado pelo Validator e pelo Judge do Generate | fora da cadeia acima — o motor é da família e o contrato v1 já existe |

## Estado

**Próximo: LAB-02.**

- **LAB-07 — concluído em 13/09/2026.** O motor do laboratório de parcelamento
  atravessa a esteira inteira pelo contrato de motor v1, nas três glebas, com
  veredito **"geometria utilizável: SIM COM RESSALVAS"**. 47 variantes julgadas,
  28 401 lotes, 4 132 violações — muito desiguais entre os dez partidos
  (`pente` 0,06 %, `cluster` 78 %, `organico` 140 %). Ver
  `docs/relatorios/LAB-07.md`. Ele entrou fora da cadeia LAB-00→LAB-06: é motor
  da família, o contrato v1 já existia, e não dependia do LAB-02.
...
- **LAB-02 — liberado, e é o próximo.** A condição era "LAB-01 devolver geometria
  utilizável", e ele devolveu. Duas coisas já vão medidas e o esperam: 38 % do
  comprimento de via nasce fora da gleba, e a rampa estoura **nos cruzamentos**
  (nunca ao longo da via). As restrições já viajam carregadas no `Terreno`.
  **O LAB-07 adiantou trabalho para ele:** a esteira contrato → motor →
  Validator → Judge já existe e está testada, o recorte de eixo viário pelo
  perímetro já está escrito (`external-engines/testfit/adapter/src/aparo.ts`,
  para copiar ou generalizar), e os nove tipos de violação do Validator já foram
  exercitados de verdade. Ver `docs/relatorios/LAB-07.md`, §14.
